...
export function generateHTML(outline, presetId) {
    const design = outline.design || getTokens(presetId);

    // Auto-generate "Agency Chapter" index
    const indexItems = outline.slides
    ...
    const slides = outline.slides.map((s, i) => renderSlide(s, i, design)).join('');
...
<head>
    ...
    <title>${esc(outline.title || 'Dynamic Presentation')}</title>
    ${design.fontUrl || ''}
    <style>
        :root {
            --bg: ${design.bg || '#0a0a0f'};
            ...
            <div class="wide-wrap">
                ...
                <div class="index-layout">
                    <div class="index-header">
                        ...
                        <p class="reveal" data-d="100">Tracking the narrative DNA through ${outline.slides.length} key evolutions.</p>
                    </div>
        ...
        <section class="slide slide--footer">
            <div class="wide-wrap reveal">
                ...
                <div class="editorial-header" style="margin-top: 5rem;">
                    <span class="editorial-label">${esc(outline.title || 'DESIGN MONSTER')}</span>
...
function renderSlide(s, idx, design) {
    ...
    if (renderer) {
        html = (s.type === 'title') ? renderer(s, id, idx) : renderer(s, id);
    } else {
        html = `<section class="slide" id="${id}"><h2>Missing Archetype: ${s.type}</h2></section>`;
    }
